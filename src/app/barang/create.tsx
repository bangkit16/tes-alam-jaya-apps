...
export default function Edit() {
  ...
  const { mutate, status } = useMutation({
    mutationFn: async (product: barangType) => addProduct(product),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['products'] });
      router.push('/barang');
      Toast.show({
        type: 'success',
        text1: 'Barang berhasil ditambahkan',
      });
    },
  });
  ...
  return (
    <SafeAreaView className='flex-1 w-full bg-white'>
      ...
      <View className='items-center justify-center p-5 '>
        <Text className='text-2xl font-bold text-green-900'>Tambah Barang</Text>
        <Text className='text-base text-gray-600'>Silahkan masukkan data barang: </Text>
      </View>
          ...
          <CardContent className='space-x-2 flex-row w-full'>
            <View className='flex-1 gap-2'>
              <View className=' mb-6'>
                ...
                />
                {errors.nama_barang && <Text className='text-red-500 text-xs mt-1 font-poppins'>{errors.nama_barang.message}</Text>}
              </View>
              <View className=' mb-6'>
                ...
                />
                {errors.kode_barang && <Text className='text-red-500 text-xs mt-1 font-poppins'>{errors.kode_barang.message}</Text>}
              </View>
              <View className=' mb-6'>
                ...
                />
                {errors.harga && <Text className='text-red-500 text-xs mt-1 font-poppins'>{errors.harga.message}</Text>}
              </View>
              <View className=' mb-6'>
                ...
                />
                {errors.stok && <Text className='text-red-500 text-xs mt-1 font-poppins'>{errors.stok.message}</Text>}
              </View>
            </View>
          </CardContent>
        </Card>
